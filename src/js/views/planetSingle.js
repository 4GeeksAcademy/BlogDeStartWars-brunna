...
export const PlanetSingle = props => {
    ...
    console.log("mostrami i parametri dei pianeti", params);
    ...
    useEffect(() => {
        console.log("muestra planets planetsDetails")

        fetch("https://www.swapi.tech/api/planets/" + params.planets_uid)
        .then((response) => response.json())
        // .then((data) => console.log(data))
        .then((data) => {
            setplanetsDetails
            (data.result.properties);
            setPlanetsDescription(data.result.description);
        });
